...
#     response = client.get(reverse('submitted_issue'))
#     assert response.status_code == 200

# @pytest.mark.django_db
# def test_assigned_issues_staff(client, django_user_model):
#     staff_user = django_user_model.objects.create_user(username='staffuser', password='password', is_staff=True)
...
#     response = client.post(reverse('register'), {'username': 'newuser', 'password1': 'password', 'password2': 'password'})
#     assert response.status_code == 302  # Redirect to home
...
#     client.login(username='staffuser', password='password')
#     response = client.post(reverse('update_issue', args=[issue.id]), {'title': "Updated Issue", 'status': "closed"})
